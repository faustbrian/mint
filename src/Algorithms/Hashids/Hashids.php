...
final class Hashids implements HashidsInterface
{
    ...
    public function __construct(
        ...
        $saltEncoding = mb_detect_encoding($salt, mb_detect_order(), true);
        $convertedSalt = mb_convert_encoding($salt, 'UTF-8', $saltEncoding !== false ? $saltEncoding : 'UTF-8');
        $this->salt = $convertedSalt !== false ? $convertedSalt : $salt;
        $alphabetEncoding = mb_detect_encoding($alphabet, mb_detect_order(), true);
        $convertedAlphabet = mb_convert_encoding($alphabet, 'UTF-8', $alphabetEncoding !== false ? $alphabetEncoding : 'UTF-8');
        $alphabet = $convertedAlphabet !== false ? $convertedAlphabet : $alphabet;
    ...
    public function encode(...$numbers): string
    {
        ...
        /** @var array<int, int|string> $numbers */
        ...
        foreach ($numbers as $i => $number) {
            $numbersHashInt += $this->math->intval($this->math->mod($number, (int) $i + 100));
        }
        ...
        foreach ($numbers as $i => $number) {
            ...
            if ((int) $i + 1 >= $numbersSize) {
                continue;
            }

            $numberMod = $this->math->intval($number) % (mb_ord($last, 'UTF-8') + (int) $i);
            $sepsIndex = $this->math->intval($this->math->mod($numberMod, mb_strlen($this->seps)));
